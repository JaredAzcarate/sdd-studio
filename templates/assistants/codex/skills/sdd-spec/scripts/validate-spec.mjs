...
function extractDomains(specPath) {
  ...
  for (const file of files) {
    ...
    if (match) {
      ...
      errors.push(`Invalid domain file name: domain/${file} (expected <domain>-domain.md)`);
    }
  }
...
function main() {
  ...
  if (domains.size === 0) {
    warnings.push("No domains found in spec/domain/ (expected <domain>-domain.md files)");
  }
  ...
  for (const { folder, suffix } of CATEGORIES) {
    ...
    for (const file of files) {
      ...
      const domain = validateFileName(folder, suffix, file);
      if (domain) {
    ...
    for (const domain of domainsInFolder) {
      if (!domains.has(domain) && suffix !== "domain") {
        warnings.push(`Orphan file: ${folder}/${domain}-${suffix}.md (no matching domain in domain/)`);
      }
    }
  }
